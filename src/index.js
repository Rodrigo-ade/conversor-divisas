...
function obtenerCambios() {
  ...
  if (!hayErrores) {
    ...
    const URL_CAMBIOS = `https://api.exchangerate.host/${fecha}?base=${base}&amount=${monto}`;
    fetch(URL_CAMBIOS)
      .then((respuesta) => respuesta.json())
...
function cargandoResultados(cargando = true) {
  const contenedorCargando = document.querySelector('#cargando');
  if (cargando) {
...
function eliminarDivisasAnteriores() {
  const contenedorDivisas = document.querySelector('#lista-conversiones');
